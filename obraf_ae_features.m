...
for dataset_num = 1:n_folders
	...
	for num_CV=1:numOffold
        ...
        for nt=1:length(NN)
            for n_activation=1:9
                ...
                for i=1:4
                        ...
                        [trainX_val,testX_val] = Extend_AE_features(trainX_val,testX_val,option_temp);
        ...
        [trainX,testX] = Extend_AE_features(trainX,testX,option);
    ...
    mean(ACC)
    %% save
    results.Enhanced_Mean_AccSP = ACC;
	...
	C = {dataset_name, round(100*mean(results.Enhanced_Mean_AccSP),2)};
